...
boolean TMRpcm::wavInfo(const char* filename){
  ...
  for (byte i =0; i<4; i++){
    ...
    //SAMPLE_RATE = sFile.read();							// Speed MOD
    //SAMPLE_RATE = sFile.read() << 8 | SAMPLE_RATE;		// Speed MOD
	orgsamplerate = sFile.read();							// Speed MOD
	orgsamplerate = sFile.read() << 8 | orgsamplerate;		// Speed MOD
...
void TMRpcm::play(const char* filename){
	play(filename,0);
}
...
void TMRpcm::speed(int sampleRate){ 					// Speed MOD
...
byte TMRpcm::getInfo(const char* filename, unsigned char* tagData, byte infoNum){
	byte gotInfo = 0;
...
byte TMRpcm::listInfo(const char* filename, unsigned char* tagData, byte infoNum){
...
byte TMRpcm::id3Info(const char* filename, unsigned char* tagData, byte infoNum){
	return metaInfo(1, filename, tagData, infoNum);
}


#if !defined (SDFAT)
boolean TMRpcm::searchMainTags(File xFile, const char *datStr){
	xFile.seek(36);
#else
unsigned long TMRpcm::searchMainTags(SdFile xFile, const char *datStr){
	xFile.seekSet(36);
#endif
		boolean found = 0;
...
byte TMRpcm::metaInfo(boolean infoType, const char* filename, unsigned char* tagData, byte whichInfo){
